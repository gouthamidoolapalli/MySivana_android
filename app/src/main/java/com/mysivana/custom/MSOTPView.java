...
public class MSOTPView extends LinearLayout {
    private EditText currentlyFocusedEditText;
    private List<EditText> editTexts = new ArrayList<>();
    private int length;

    public MSOTPView(Context context) {
        super(context);
        init(null);
    }

    public MSOTPView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public MSOTPView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs);
    }

    private void init(AttributeSet attrs) {
        TypedArray styles = getContext().obtainStyledAttributes(attrs, R.styleable.MSOTPView);
    ...
    private void styleEditTexts(TypedArray styles) {
        length = styles.getInt(R.styleable.MSOTPView_length, 4);
    ...
    private void generateViews(TypedArray styles) {
        if (length > 0) {
            for (int i = 0; i < length; i++) {
                MSEditText editText = new MSEditText(getContext());
                editText.setId(i);
                editText.setSingleLine();
                editText.setMaxLines(1);
                editText.setGravity(Gravity.CENTER);
                editText.setTypeface();
                editText.setTextSize(getPixels(12));
                ...
                params.setMargins(getPixels(10), getPixels(10), getPixels(10), getPixels(10));
                editText.setLayoutParams(params);
                int textColor = styles.getColor(R.styleable.MSOTPView_android_textColor, Color.BLACK);
                int backgroundColor =
                        styles.getColor(R.styleable.MSOTPView_text_background_color, Color.TRANSPARENT);
                ...
                editText.setInputType(styles.getInt(R.styleable.MSOTPView_android_inputType,
